---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

Education
======
* **Ph.D. in Internal Medicine** (Provisionally Admitted)
  - Capital Medical University, Sept 2026 (Expected)
  
* **B.Med. in Clinical Medicine**
  - Huaqiao University, Sept 2021 - June 2026
  - CET-6: 513

Publications
======
1. **Bao, R.**, Li, H., Li, S., Su, M., & Li, W. (2025). Benfuracarb impairs zebrafish swim bladder development via the JNK2 pathway mediated inhibition of autophagy. *Pesticide Biochemistry and Physiology*, **209**, [cover article]. IF=4.2, JCR Q1

2. Su, M., **Bao, R.**, Wu, Y., Gao, B., Xiao, P., & Li, W. (2023). Diafenthiuron causes developmental toxicity in zebrafish (*Danio rerio*). *Chemosphere*. IF=8.9, JCR Q1

3. Huang, Y., **Bao, R.**, Guo, S., Xiao, P., Fu, H., & Li, W. (2025). Advanced oxidation processes for the degradation of tralomethrin: Impacts on zebrafish embryonic development. *Journal of Environmental Management*. IF=8, JCR Q1

4. Su, M., **Bao, R.**, Gao, B., Liao, X., Xiao, P., & Li, W. (2024). Embryotoxicity of diafenthiuron to zebrafish (*Danio rerio*) after advanced oxidation treatment. *Water*, **11**(3). IF=3, JCR Q2

5. Xu, Y., Gao, B., **Bao, R.**, Shi, Y., Li, W., & Xiao, P. (2026). Intergenerational effects of parental dimefluthrin and microcystins co-exposure on zebrafish: Impaired embryonic and larval development. *Comparative Biochemistry and Physiology. Toxicology & Pharmacology*. IF=4.3, JCR Q1

6. **Bao, R.**, et al. The potential mechanisms of compound xuanju capsule for treating asthenozoospermia. (In preparation)

Patents
======
1. **Bao, R.**, Liu, Z., Wang, F., Su, M., Huang, J., Huang, Y., Xie, Q. Method and System for Pesticide Target Protein Screening Based on the Entropy-Weighted TOPSIS Algorithm. Chinese Patent Application CN119181425A

2. Li, W., Zhou, L., Zhou, J., **Bao, R.**, Su, M. Method for Construction and Application of an Animal Model of Asthenozoospermia. Chinese Patent Application CN117467709A

3. Liu, Z., **Bao, R.**, Wu, M., Wang, F., Sun, H., Lei, H. Feature Correlation-Based Detection Method for Osteosarcoma Lesion Regions in CT Images. Chinese Patent Application CN118587217B

Research Experience
======
* **International Undergraduate Summer School on Frontiers of Life Sciences**
  - Westlake University, 2023

* **Summer Training Program in Medical Sciences**
  - Chinese Institutes for Medical Research (CIMR), Capital Medical University, 2024

Technical Skills
======

**Programming Languages**
: Java, Python, R, MATLAB
: MySQL database querying
: Statistical analysis (SPSS, R, Python)

**Computational Biology & Bioinformatics**
: Agent-based modeling (Netlogo)
: Molecular docking (Schrödinger)
: Molecular dynamics simulations (GROMACS)
: Network analysis (Cytoscape, Gephi)
: Linux operating systems

**Omics Data Analysis**
: Bulk RNA-seq analysis (DESeq2)
: Single-cell RNA-seq analysis (Seurat)
: Single-cell ATAC-seq analysis (Signac, ArchR)

**Scientific Visualization**
: PowerPoint, Adobe Illustrator
: GraphPad Prism, R ggplot2

Honors & Awards
======

**Mathematical Modeling & Data Analysis Competitions**
* **National First Prize**, MathorCup Mathematical Modeling Competition (2022)
  - "Base Station Location Optimization Model Based on Multi-objective Programming and Cluster Analysis"

* **Provincial Second Prize**, China Undergraduate Mathematical Contest in Modeling (2022)
  - "Study on Passive UAV Localization Using Improved Simulated Annealing for TDOA System"

* **National First Prize**, MathorCup Big Data Analytics Competition for Universities (2023)
  - "Customer Satisfaction Analysis and Prediction Model Based on Feature Engineering and Stepwise Regression"

* **National Second Prize**, "Challenge Cup" Chinese College Student Entrepreneurship Plan Competition (2024)
  - "Multimodal Adaptive Transformer Network for Intelligent Depression Level Assessment"

* **National Second Prize**, Asia and Pacific Mathematical Contest in Modeling (APMCM, 2024)
  - "Sustainable Development of China's Pet Industry: An International Trade Analysis"

**Life Science Competition**
* **National First Prize (Team Leader)**, China Undergraduate Life Sciences Contest (2025)
  - "Mechanistic Study of JNK2-Mediated Autophagy Inhibition in Propoxur-Induced Swim Bladder Developmental Defects in Zebrafish"

**Academic Honors & Scholarships**
* Outstanding Student, Huaqiao University (2021-2022, 2022-2023)
* First-Class Scholarship, Huaqiao University (2021-2022, 2022-2023)
* President's Special Award, Huaqiao University (2025)
